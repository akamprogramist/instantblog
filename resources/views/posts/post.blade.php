<tr>
    ...
    <td>{{ shortNumber($post->counter) }}</td>
    <td><a href="{{ url('/profile/' . $post->user->username) }}">{{ str_limit($post->user->name, 10) }}</a></td> 
    <td><a href="{{ url('/home/' . $post->id . '/edit') }}">@lang('admin.edit')</a></td>
    <td><a class="color-delete" href="{{ url('/home/' . $post->id) }}">@lang('admin.delete')</a></td>
</tr>